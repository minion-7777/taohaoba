...
public class N {
    ...
    public static Retrofit mRetrofit,mRetrofitThb,mRetrofitSjb,mRetrofitFeige,mRetrofitDun;
...
//            build.addInterceptor(LogInterceptor());//添加日志拦截器
            build.addInterceptor(logging);//添加日志拦截器
        ...
        mRetrofit = new Retrofit.Builder()
            ...
            .client(build.build())
            .build();
        mRetrofitThb = new Retrofit.Builder()
            .baseUrl(Constants.baseUrlThb)
            .addConverterFactory(GsonConverterFactory.create())//添加gson转换器
            .addCallAdapterFactory(RxJava2CallAdapterFactory.create())//添加rxjava转换器
            .client(CustomOkHttpClient.getClient())
            .build();
        mRetrofitDun = new Retrofit.Builder()
            .baseUrl("https://verify.dun.163.com")
            .addConverterFactory(GsonConverterFactory.create())//添加gson转换器
            .addCallAdapterFactory(RxJava2CallAdapterFactory.create())//添加rxjava转换器
            .client(build.build())
            .build();
    ...
    public static <T> T APIThb(Class<T> service) {
        initExceptionThb();
        return mRetrofitThb.create(service);
    }
    public static <T> T APISjb(Class<T> service) {
        initExceptionSjb();
        return mRetrofitSjb.create(service);
    }
    public static <T> T APIFeige(Class<T> service) {
        initExceptionFeige();
        return mRetrofitFeige.create(service);
    }
    public static <T> T APIDun(Class<T> service) {
        initExceptionDun();
        return mRetrofitDun.create(service);
    }
    ...
    private static void initExceptionThb() {
        if (null == mRetrofitThb)
            throw new NullPointerException("N is not initialized");
    }
    private static void initExceptionSjb() {
        if (null == mRetrofitSjb)
            throw new NullPointerException("N is not initialized");
    }
    private static void initExceptionFeige() {
        if (null == mRetrofitFeige)
            throw new NullPointerException("N is not initialized");
    }
    private static void initExceptionDun() {
        if (null == mRetrofitDun)
            throw new NullPointerException("N is not initialized");
    }
