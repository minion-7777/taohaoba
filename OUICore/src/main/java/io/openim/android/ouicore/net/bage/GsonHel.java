...
public class GsonHel {
     ...
     * @return
     */
    public static <T> BaseTHB<T> dataObjectThb(String jsonStr, Class<T> clazz) throws Exception {
        get();
        Type type = new ParameterizedTypeImplThb(BaseTHB.class, new Class[]{clazz});
     ...
     * @return
     */
    public static <T> BaseTHB<List<T>> dataArrayThb(String jsonStr, Class<T> clazz) throws Exception {
        get();
        // 生成List<T> 中的 Type
        Type listType = new ParameterizedTypeImplThb(List.class, new Class[]{clazz});
        // 根据List<T>生成的，再生出完整的Base<List<T>>
        Type type = new ParameterizedTypeImplThb(BaseTHB.class, new Type[]{listType});
        return JSONObject.parseObject(jsonStr, type);
    }
    ...
    public static class ParameterizedTypeImplThb implements ParameterizedType {

        private final Class raw;
        private final Type[] args;

        public ParameterizedTypeImplThb(Class raw, Type[] args) {
            this.raw = raw;
            this.args = args != null ? args : new Type[0];
        }
