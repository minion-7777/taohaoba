...
public class ConversationProvider {
    ...
    public void loadConversation(int type, long startSeq, int loadCount, final IUIKitCallback<List<ConversationInfo>> callBack) {
        TUIConversationLog.i(TAG, "loadConversation startSeq " + startSeq + " loadCount " + loadCount);

        V2TIMConversationListFilter filter = new V2TIMConversationListFilter();
        if (type == 0){
            filter.setConversationType(V2TIMConversation.CONVERSATION_TYPE_INVALID); //拉取全部会话
        }else if (type == 1){
        ...
        V2TIMManager.getConversationManager().getConversationListByFilter(filter, startSeq, loadCount, new V2TIMValueCallback<V2TIMConversationResult>() {
            ...
            public void onSuccess(V2TIMConversationResult v2TIMConversationResult) {
                ...
                if (!conversationInfoList.isEmpty()) {
                    TUIConversationLog.i(TAG,
                        "loadConversation getConversationList success first " + JSONObject.toJSONString(conversationInfoList));
                }
                isFinished = v2TIMConversationResult.isFinished();
    ...
    public void loadMoreConversation(int loadCount, IUIKitCallback<List<ConversationInfo>> callBack) {
        ...
        loadMoreConversation(0,0, loadCount, callBack);
    }

    public void loadMoreConversation(int type, int page, int loadCount, IUIKitCallback<List<ConversationInfo>> callBack) {
//        if (isFinished) {
//            return;
//        }
        loadConversation(type, page == 0 ? page : nextLoadSeq, loadCount, callBack);
    }
